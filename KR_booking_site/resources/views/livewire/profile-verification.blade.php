                ...
                @if (!$showOtpField)
                    <form wire:submit.prevent="submitDetails">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="date_of_birth" wire:model="date_of_birth" required>
                            @error('date_of_birth') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-3">
                            <label for="national_id_or_passport" class="form-label">National ID or Passport Number</label>
                            <input type="text" class="form-control" id="national_id_or_passport" wire:model="national_id_or_passport" required>
                            @error('national_id_or_passport') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                ...
                @else
                    <form wire:submit.prevent="verifyOtp">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="otp" wire:model="otp" required maxlength="4">
                            @error('otp') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>

                        <button type="submit" class="btn btn-primary">Verify OTP</button>
                        <button type="button" wire:click="$set('showOtpField', false)" class="btn btn-secondary">Back</button>
                    </form>
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                @else
                    <form wire:submit.prevent="verifyOtp">
                        <div class="mb-3">
                            <label for="otp" class="form-label">Enter OTP sent to {{ $phone_number }}</label>
                            <input type="text" class="form-control" id="otp" wire:model="otp" required maxlength="6">
                            @error('otp') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
